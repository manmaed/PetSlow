 ...
 */
public class SignFeatureRenderer extends FeatureRenderer<EntityPetSlow, ModelSlowpoke<EntityPetSlow>> {

    private final ModelSign sign = new ModelSign();
    private static final Identifier skin = new Identifier(Reference.MOD_ID, "textures/entity/sign.png");

    public SignFeatureRenderer(FeatureRendererContext<EntityPetSlow, ModelSlowpoke<EntityPetSlow>> featureRendererContext) {
    ...
    public void render(EntityPetSlow petSlow, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
        if (petSlow.isSitting()) {
            if (petSlow.getAway()) {
                ...
                sign.render(petSlow, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scale);
